...
router.post('/search', function (req, res) {
	...
	try {
		...
		/*} else if (req.body.chkProceedings && req.body.chkResearchs) {
		...
		} else if (req.body.chkProceedings && req.body.chkGroups) {
			functions.getProceedingsByItsNameTeacherOrStudents(searchValue).then(function (proceedings) {
				functions.getGroupsByItsNameTeacherOrStudents(searchValue).then(function (groups) {
					functions.getTeacherInfoByItsName(searchValue).then(function (cloud) {
						tagCloud.tagCloud(cloud, function (err, data) {
							res.render('pages/searchProceedings', { chkBoxProceedings: true, chkBoxResearchWorks: false, chkBoxGroups: true, proceedingsByName: proceedings[0][0], proceedingsByAuthor: proceedings[1][0], proceedingsByStudents: proceedings[2][0], groupsByName: groups[0][0], groupsByAuthor: groups[1][0], groupsByStudents: groups[2][0], cloud: data });
						},
							{
								classPrefix: 'btn tag tag',
								randomize: true,
		...
		} else if (req.body.chkResearchs && req.body.chkGroups) {
			functions.getResearchsByItsNameTeacherOrStudents(searchValue).then(function (researchWorks) {
				functions.getGroupsByItsNameTeacherOrStudents(searchValue).then(function (groups) {
					functions.getTeacherInfoByItsName(searchValue).then(function (cloud) {
						tagCloud.tagCloud(cloud, function (err, data) {
							res.render('pages/searchProceedings', { chkBoxProceedings: false, chkBoxResearchWorks: true, chkBoxGroups: true, researchWorksByName: researchWorks[0][0], researchWorksByAuthor: researchWorks[1][0], researchWorksByStudents: researchWorks[2][0], groupsByName: groups[0][0], groupsByAuthor: groups[1][0], groupsByStudents: groups[2][0], cloud: data });
						},
							{
								classPrefix: 'btn tag tag',
								randomize: true,
			...
			if (req.body.chkProceedings) {
				functions.getProceedingsByItsNameTeacherOrStudents(searchValue).then(function (proceedings) {
					functions.getTeacherInfoByItsName(searchValue).then(function (cloud) {
						tagCloud.tagCloud(cloud, function (err, data) {
							if (err) throw err;
							res.render('pages/searchProceedings', { chkBoxProceedings: true, chkBoxResearchWorks: false, chkBoxGroups: false, proceedingsByName: proceedings[0][0], proceedingsByAuthor: proceedings[1][0], proceedingsByStudents: proceedings[2][0], cloud: data });
						},
							{
								classPrefix: 'btn tag tag',
								randomize: true,
			...
			if (req.body.chkResearchs) {
				functions.getResearchsByItsNameTeacherOrStudents(searchValue).then(function (researchWorks) {
					functions.getTeacherInfoByItsName(searchValue).then(function (cloud) {
						tagCloud.tagCloud(cloud, function (err, data) {
							if (err) throw err;
							res.render('pages/searchProceedings', { chkBoxProceedings: false, chkBoxResearchWorks: true, chkBoxGroups: false, researchWorksByName: researchWorks[0][0], researchWorksByAuthor: researchWorks[1][0], researchWorksByStudents: researchWorks[2][0], cloud: data });
						},
							{
								classPrefix: 'btn tag tag',
								randomize: true,
			...
			if (req.body.chkGroups) {
				functions.getGroupsByItsNameTeacherOrStudents(searchValue).then(function (groups) {
					functions.getTeacherInfoByItsName(searchValue).then(function (cloud) {
						tagCloud.tagCloud(cloud, function (err, data) {
							if (err) throw err;
							res.render('pages/searchProceedings', { chkBoxProceedings: false, chkBoxResearchWorks: false, chkBoxGroups: true, groupsByName: groups[0][0], groupsByAuthor: groups[1][0], groupsByStudents: groups[2][0], cloud: data });
						},
							{
								classPrefix: 'btn tag tag',
								randomize: true,
